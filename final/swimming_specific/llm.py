"""Everything that talks to the model's HTTP API — OpenAI, or any endpoint that
speaks the same dialect. Which one is decided at startup (config.choose_provider)
and read here per call, never captured at import time."""
import base64
import hashlib
import json
import threading
...
from config import (CACHE_DIR, HTTP_RETRIES, MAX_OUT, MAX_OUT_CEILING,
                    SHOW_TOKENS, api_key, base_url, header_model, judge_model,
                    model)
from log import log
...
def usage_reset() -> dict:
    """Clear the running totals and hand back what they were.

    USAGE is module-level, so a batch of PDFs would otherwise report file 7's
    usage as everything spent since file 1. batch.py resets between files and
    adds the returned figures up itself."""
    with _usage_lock:
        was = {k: dict(v) for k, v in USAGE.items()}
        USAGE.clear()
        return was


def usage_summary(totals: dict = None) -> str:
    """Per-call-kind totals. The live ones, or a set handed back by usage_reset."""
    totals = USAGE if totals is None else totals
    if not totals:
