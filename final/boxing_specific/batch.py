...
def link_out_paths(urls, out_dir) -> dict:
    ...
    that is what distinguishes two sites that both publish a `draw.pdf`."""
...
def run_folder(in_dir, out_dir="output", only=None, recursive=False,
    ...
    Returns {"done": {path: (standings df, bouts df, stats)}, "failed":
...
def _summary(done, failed, skipped, usage, names, out_dir) -> None:
    ...
    bouts = sum(len(b) for _, b, _ in done.values() if b is not None)
    log.warning(f"{rows} rows and {bouts} bouts -> {out_dir}/")
